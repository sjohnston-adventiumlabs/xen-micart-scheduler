...
uint64_t twiddle_flag = 1;
#define twiddle_mask_even 0xFFFFFFFFFFFFFFFE
#define twiddle_mask_odd  0x0000000000000001
...
static void hpet_set_timer(HPETState *h, unsigned int tn)
{
    ...
    if ( !err ) {
      if ( !hpet_enabled(h) ) {
          printk("HPET not enabled.\n");
          err = 1;
      }
      if ( !timer_enabled(h, tn) ) {
          printk("HPET Timer %d not enabled.\n", tn);
          err = 1;
          return;
      }
    }
     ...
     */
    // System errors on 64 bit diff, use 32 bit signed integer
    //if ( (int64_t)diff < 0 )
    if ( (int32_t)diff < 0 )
...
// HPET pukes when queried with this in... hope it doesn't break
...
void hpet_init(struct vcpu *v)
{
    ...
    h->hpet.config |= HPET_CFG_ENABLE;

    for ( i = 0; i < HPET_TIMER_NUM; i++ )
    {
        h->hpet.timers[i].config = HPET_TN_ENABLE |
...
// Trying to set HPET timer 2 to contain the time when the slice will be
// swapped out.
void micart_set_hpet(struct vcpu *v, uint32_t diff) {
    uint64_t cmp;
    HPETState *hp;

    hp = vcpu_vhpet(v);
    // Removed to set up masking of last cmp bit.
    //cmp = hp->hpet.mc64;
    //cmp += diff;
    cmp = diff;
    
    if (twiddle_flag) {
	cmp = (cmp | twiddle_mask_odd);
	//printk("\ntwiddle_flag = %lu\n", twiddle_flag);
	//printk("cmp = %lu\n", cmp);
	twiddle_flag = 0;
    }
    else{
	cmp = (cmp & twiddle_mask_even);
	//printk("\ntwiddle_flag = %lu\n", twiddle_flag);
	//printk("cmp = %lu\n", cmp);
	twiddle_flag = 1;
    }
    ...
    hp->hpet.comparator64[2] = cmp;
    if ( timer_is_32bit(hp, 2) )
	cmp = (uint32_t)cmp;
    hp->hpet.timers[2].cmp = cmp;
  // If you want to test, do so via known value. (comp64 is what is read)
  // hp->hpet.timers[2].cmp = 12345;
  // hp->hpet.comparator64[2] = 12345;
}
