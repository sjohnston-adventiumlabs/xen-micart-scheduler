...
SUBCOMMAND_HELP = {
    ...
    'micart'      : ('action [options]', 'get/set micart schedule'),
...
SUBCOMMAND_OPTIONS = {
    ...
    'micart': (
       ('-D DOMAIN', '--domain=DOMAIN', 'domain to modify'),
       ('-v VCPUID', '--vcpu=VCPUID',   'virtual cpu within domain (int)'),
       ('-p PCPUID', '--pcpu=PCPUID',   'physical cpu (int)'),
       ('-d DUR',    '--duration=t',    'slice duration ns,us,ms'),
       ('-w PHASE',  '--phase=t',       'slice offset in frame'),
       ('-f FRAME',  '--frame=t',       'frame duration'),
       ('-s',        '--slacktime',     'the vcpu accepts slacktime'),
       ('-r',        '--realtime',      'the domain is realtime'),
       ('-h DOMAIN', '--helper=DOMAIN', 'helper domain for -D'),
    ),
    'list': (
...
def micart_usage():
    ...
    print "\nmicart usage:"
    print "   xm micart action [args*]\n"
    print "valid actions are:"
    print "   slice [args] : add a slice to new schedule"
    print "   set [args] : set various options"
    ...
    print "   --pcpu=p  : index of physical cpu on which to run"
    print "   --vcpu=v  : index of vcpu within domain"
    print "   --duration=d  : duration of slice"
    print "   --phase=w  : offset of slice start into frame"

    print "\nset args:"
    print "   --pcpu=p  : index of physical cpu on which to run"
    print "   --frame=t  : frame duration for the pcpu p"
    print "   --vcpu=v  : index of vcpu within domain"
    print "   --slacktime : the vcpu v accepts slack (extra) time"
    print "   --domain=D  : domain (name or number)"
    print "   --helper=h  : domain id of a helper for domain D"
    print "   --realtime : the domain D is a realtime domain"

    print "\ndefault args:"
    print "   --domain=D  : domain (name or number)"
    
    print "\nTime units are ms by default, however may be altered by suffix, e.g."
    print "    325u  == 325 microseconds"
    print "   8000n  == 8000 nanoseconds"

    print "\n"
    sys.exit(1)


# take a string x that may have a time suffix and return 
# an integer number of nanoseconds. With no suffix, milliseconds
# are assumed.  Valid suffixes are:
#    s  - seconds
#  m,ms - milliseconds
#  u,us - microseconds
#  n,ns - nanoseconds
...
def time_as_ns(x):
    ...
    if x.isdigit():
        return ms_to_ns( int(x) )

    if x.endswith(('n','ns')):
        return int(x[0:x.index('n')])

    if x.endswith(('m','ms')):
        return ms_to_ns( int(x[0:x.index('m')]) )

    if x.endswith(('u','us')):
        return us_to_ns( int(x[0:x.index('u')]) )

    if x.endswith('s'):
        return s_to_ns( int(x[0:x.index('s')]) )

    print "'"+x+"' is not a valid time specification."
    sys.exit(1)




def xm_sched_micart(args):
    """Get/Set options for MiCART Scheduler."""
    domid = None
    vcpu = None
    pcpu = None
    duration = None
    ...
    def do_slice():
        ...
        if None==pcpu:
            ...
            sys.exit(1)
        if None==vcpu:
            print "Error: slice must specify a vcpu"
            sys.exit(1)
        if None==duration:
            print "Error: slice must specify a duration"
            sys.exit(1)
        if None==phase:
    ...
    #Print slice parameters input
        ...
        try:
            ...
            result = server.xend.domain.sched_micart_slice(XEN_MIC_FUNCTION_SLICE,domid,pcpu,vcpu,duration,phase)
            if (result == 0):
                print "Slice created\n"
            else:
    ...
    def do_set():
        ...
        if ( None!=frame and None==pcpu ):
                ...
                sys.exit(1)
        if ( None!=slacktime and (None==vcpu or None==domid) ):
            print "Error: to set slacktime you must specify a domain and vcpu"
            sys.exit(1)
        if ( 1==realtime and None==domid ):
            print "Error: to set realtime you must specify a domain"
            sys.exit(1)
        if ( None!=helper and None==domid ):
            print "Error: to set helper domain you must specify a domain"
            sys.exit(1)
        if ( None != helper and (helper == domid)):
            print "Error: a domain cannot be its own helper, clever as that sounds."
            sys.exit(1)
        ...
        try:
            ...
            if(helper):
                result = server.xend.domain.sched_micart_set(XEN_MIC_FUNCTION_SET, domid,pcpu,vcpu,options,frame,helper)
            else:
                result = server.xend.domain.sched_micart_set(XEN_MIC_FUNCTION_SET, domid,pcpu,vcpu,options, frame)
    ...
    def do_setslice():
        ...
        try:
            ...
            function = XEN_MIC_FUNCTION_OPTS
            if(helper):
                result = server.xend.domain.sched_micart_set(function,domid,pcpu,vcpu,options, 0, helper)
            else:
                result = server.xend.domain.sched_micart_set(function,domid,pcpu,vcpu,options)
    ...
    def do_swap():
        ...
        try:
            ...
            result = server.xend.domain.sched_micart_set(XEN_MIC_FUNCTION_SWAP)
            if (result == 0):
    ...
    def do_clear():
        ...
        try:
            ...
            result = server.xend.domain.sched_micart_set(XEN_MIC_FUNCTION_CLEAR)
            if (result == 0):
    ...
    def do_default():
        ...
        try:
            ...
            if (domid == 0):
                ...
                a = 1
                function = XEN_MIC_FUNCTION_SLICE
                while (a <= domid):
                    ...
                    if (result['domid'] == a):
                        ...
                        function = XEN_MIC_FUNCTIONS_OPTS
                        result = server.xend.domain.sched_micart_set(function,domid,pcpu,vcpu,options)
                    if (result == 0):
                        print "Schedule cleared\n"
                    else:
    ...
    def do_get():
        ...
        try:
            if serverType == SERVER_XEN_API:
                ...
                function = XEN_MIC_FUNCTION_GET
                vcpu = 0
                print "function == %d\n" % (function)
                result = server.xend.domain.sched_micart_get(function, domid, pcpu, vcpu)
                #print "result == %s\n" % result['domain_id']
                print "result == %s\n" % result
                #print "phase == %s\n" % result['phase']
    ...
    if operation not in ["slice","set","get","swap","clear","default","setslice"]:
        print "\nError: '"+operation+"' is not a valid micart action."
        micart_usage()

    try:
        opts, params = getopt.getopt(args, "D:v:p:d:w:f:s:r:h:",
            ["domain=", "vcpu=", "pcpu=", "duration=", "phase=", "frame=",
             "slacktime", "realtime", "helper="])
    except getopt.GetoptError, opterr:
        err(opterr)
        micart_usage()
    ...
    for o, a in opts:
        if o in ["-D", "--domain"]:
            ...
            duration = time_as_ns(a);
        elif o in ["-w", "--phase"]:
            phase = time_as_ns(a);
        elif o in ["-f", "--frame"]:
            frame = time_as_ns(a);
        elif o in ["-s", "--slacktime"]:
            slacktime = 1
            options |= XEN_MIC_OPTION_SLACK
        elif o in ["-r", "--realtime"]:
            realtime = 1
            options |= XEN_MIC_OPTION_REALTIME
        elif o in ["-h", "--helper"]:
            helper = int(a)
            options |= XEN_MIC_OPTION_HELPER    
    if (operation == 'slice'):
        do_slice()
    elif (operation == 'set'):
        do_set()
