...
def xm_sched_micart(args):
	...
	try:
	    ...
	    result = server.xend.domain.sched_micart_set(domid, pcpu, frame, vcpu, slacktime, 
						     helper, realtime)
    ...
    def do_clear(domid, vcpu, pcpu, duration, phase, frame, slacktime, realtime):
        print "\nClear new schedule.\n"
        ## TBD
	...
	try:
		...
		print "vcpu == %s" % vcpu
		b = server.xend.domain.sched_micart_get(vcpu, domid)
		print "\ninfo == %s\n" % b



		#print "vcpu == %s" % vcpu
		#a = {'domid':domid, 'vcpu':vcpu}
		#b = server.xend.domain.sched_micart_get(domid, a)
		#b = xc.sched_micart_domain_get(0, 11, 12, 13, 14, 15)
		#print "\ninfo == %s\n" % b


		#dict
		#a = {'domid':domid, 'vcpu':vcpu}
		#b = server.xend.domain.sched_micart_get(a['domid'], a)
		#print "\ninfo ==", b
		#list
		#a = [domid, vcpu]
		#b = server.xend.domain.sched_micart_get(domid, a)
    ...
    if operation not in ["slice","set","get","swap","clear","default"]:
	...
	#domid = 1
	#print "(external clear before) domain == %s" % domid
        args = do_clear(domid, vcpu, pcpu, duration, phase, frame, slacktime, realtime)
	domid = args[0]
	vcpu = args[1]
	print "(external clear after) domain == %s\nvcpu == %s" % (domid, vcpu)
