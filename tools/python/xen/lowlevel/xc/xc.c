...
/* The Python argument conversion codes of possible use with scheduler parameters:
 * i  int
 * I  uint32
 * H  unit16
 * L  int64
...
/* Any of these keyword arguments may be set:
 *      "function", "domid", "options", "helper", "pcpu", "vcpu",
 *      "period", "phase", "duration"
 * The only mandatory argument is function, which must be one of
 *   XEN_MIC_FUNCTION_{slice,clear,swap,opts}
 */
static PyObject *pyxc_sched_micart_domain_set(XcObject *self,
                                              PyObject *args,
                                              PyObject *kwds)
{
    uint32_t domid=0;
    ...
    uint32_t pcpu=0;
    uint32_t vcpu=0;
    uint64_t period=0;          /* aka "frame" */
    uint64_t phase=0;
    uint64_t duration=0;
    static char *kwd_list[] = {
        "function", "domid", "options", "helper", "pcpu", "vcpu",
        "period", "phase", "duration", NULL };
    static char kwd_type[] = "I|IIIIIKKK";
    struct xen_domctl_sched_micart sdom;
    
    if( !PyArg_ParseTupleAndKeywords(args, kwds, kwd_type, kwd_list, 
            			     &function, &domid, &options, &helper, &pcpu, 
				     &vcpu, &period, &phase, &duration ) ) {
        return NULL;
    }
    sdom.function = function;
    sdom.options  = options;
    sdom.helper   = helper;
    sdom.pcpu     = pcpu;
    sdom.vcpu     = vcpu;
    sdom.period   = period;
    sdom.phase    = phase;
    sdom.duration = duration;

    if ( xc_sched_micart_domain_set(self->xc_handle, domid, &sdom) != 0 ) {
        return pyxc_error_to_exception();
    }
    Py_INCREF(zero);
    return zero;
 ...
 * The function is a mandatory argument; function must be
 * one of:  XEN_MIC_FUNCTION_{slice,opts}
 * The "new" arg is optional--if set to nonzero then the new schedule is
 * queried, otherwise the current (running schedule) is queried.
 *
 * Case of function "opts":
 *   caller specifies:  {domid, vcpu, new=0}
 *   return struct has: {options, helper}
 *   The options (XEN_MIC_OPTION_x) will reflect whether:
 *     - HELPER: the domain has a helper (in which case helper is domid)
 *     - REALTIME: the domain is marked as "realtime"
 *     - SLACK: the indicated vcpu of the domain accepts slacktime
 *
 * Case of function "slice":
 *   caller specifies:  {pcpu, slice, new=0}
 *   The "slice" is used as an index into the slices for pcpu's schedule
 *   of the desired slice for the query.
 *
 *   return struct has: {pcpu, vcpu, domid, period, phase, duration}
 *   where domid identifies the domain of the slice, vcpu is the
 *   index of that domain's vcpus, and period/phase/duration are the
 *   temporal parameters of the indexed slice. Note that domid is
 *   actually passed via the "helper" member of the domctl union.
 *
 * On error, NULL is returned.
 */
static PyObject *pyxc_sched_micart_domain_get(XcObject *self, PyObject *args,
                                       PyObject *kwds)
{
    uint32_t function=0;
    uint32_t domid=0;
    ...
    sdom.function = function;
    // ':' was '|'
    sdom.options = (new ? XEN_MIC_OPTION_NEW : 0);

    if (function == XEN_MIC_FUNCTION_slice) { /* get SLICE */
        sdom.pcpu = pcpu;
        sdom.vcpu = slice;
        
    } else if (function == XEN_MIC_FUNCTION_opts) { /* get OPTS */
        sdom.vcpu = vcpu;
    ...
    /* Return arg marshalling: 2 cases
     */
...
static PyMethodDef pyxc_methods[] = {
    ...
    /* MiCART */
    { "sched_micart_domain_set", /* name to appear in python */
      (PyCFunction)pyxc_sched_micart_domain_set, /* C function (above) */
      METH_KEYWORDS, "\n"                        /* flag: keyword arg passing */
      "Set the scheduling parameters when running with the MiCART scheduler.\n"
      " function     [int]:   XEN_MIC_FUNCTION_slice,_opts,_clear,_swap\n"
      " domid        [int]:   domain id to set\n"
      " pcpu         [int]:   id of the PCPU whose schedule will be altered\n"
      " vcpu         [int]:   vcpu id within domain\n"
      " period       [int]:   frame length in nanoseconds"
      " phase        [int]:   slice start offset within frame in nanoseconds"
      " duration     [int]:   slice duration in nanoseconds"
      " options      [int]:   bit flags\n"
      " helper       [int]:   domid of helper domain, optional\n"
      "Returns: [int] 0 on success; -1 on error.\n" },

    /* MiCART */
    { "sched_micart_domain_get",
      (PyCFunction)pyxc_sched_micart_domain_get,
      METH_KEYWORDS, "\n"        /* flag: keyword arg passing */
      "Inspect the schedule when running with the MiCART scheduler.\n"
      " function     [int]:   XEN_MIC_FUNCTION_slice or XEN_MIC_FUNCTION_opts\n"
      " domid        [int]:   domain id for opts function\n"
      " vcpu         [int]:   vcpu id within domain for opts function\n"
      " pcpu         [int]:   id of the PCPU to interrogate\n"
      " slice        [int]:   index of a slice in PCPU's schedule\n"
      " new          [int]:   if nonzero then query new schedule not current\n"
      "Returns:   [dict]\n"
      " vcpu         [int]:   vcpu id within domain for opts function\n"
      " pcpu         [int]:   id of the PCPU to interrogate\n"
      " period       [int]:   frame length in nanoseconds"
...
PyMODINIT_FUNC initxc(void)
{
    ...
    /*  some micart scheduler constants: */
    PyModule_AddIntConstant(m, "XEN_SCHEDULER_MICART", XEN_SCHEDULER_MICART);
    PyModule_AddIntConstant(m, "XEN_MIC_FUNCTION_slice", XEN_MIC_FUNCTION_slice);
    PyModule_AddIntConstant(m, "XEN_MIC_FUNCTION_opts", XEN_MIC_FUNCTION_opts);
    PyModule_AddIntConstant(m, "XEN_MIC_FUNCTION_clear", XEN_MIC_FUNCTION_clear);
    PyModule_AddIntConstant(m, "XEN_MIC_FUNCTION_swap", XEN_MIC_FUNCTION_swap);
    PyModule_AddIntConstant(m, "XEN_MIC_FUNCTION_default", XEN_MIC_FUNCTION_default);
}
