...
class SrvDomain(SrvDir):
    ...
    #####################################################################################	
    ## MiCART
    ## These call the XendDomain methods from http server 
    ## The OP_DOMAIN prefix in the name is important for xmlrpc dynamic method dispatching		
    def op_domain_sched_micart_get(self, _, req):
    ...
    def op_domain_sched_micart_set(self, _, req):
        fn = FormFn(self.xd.domain_sched_micart_set,
        	   [['function', 'str'],
		    ['domid', 'str'],
                    ['options', 'str'],
                    ['helper', 'str'],
